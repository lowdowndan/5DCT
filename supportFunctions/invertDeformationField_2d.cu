...
// getInverse
__global__ void getInverse(cudaTextureObject_t XTex, cudaTextureObject_t YTex, float* hd_iX, float* hd_iY, size_t dimX, size_t dimY, size_t dimZ, size_t numVoxels, size_t numIterations){
	...
	ix = tex3D<float>(XTex, (x + ix + 0.5f), (y + iy + 0.5f), (z + 0.5f)) * -1;
	iy = tex3D<float>(YTex, (x + ix + 0.5f), (y + iy + 0.5f), (z + 0.5f)) * -1;
	}
...
if (nrhs != 3 || nlhs > 2){
	mexErrMsgIdAndTxt("MATLAB:badInput","Usage: [iX, iY] = invertDeformationField_2d(dfX,dfY, numIterations)\n");
}
...
for (int i=0; i < (nrhs - 1); i++){
	...
	mexErrMsgIdAndTxt("MATLAB:badInput","dfX, and dfY must be of data type single.\n");
	}

	sizeCheck = mxGetNumberOfDimensions(prhs[i]);
	if (sizeCheck != 2){
	mexErrMsgIdAndTxt("MATLAB:badInput","dfX, dfY must be 2D matrices.\n");
	}
	...
	for (int j = 0; j < 2; j++){
		...
		mexErrMsgIdAndTxt("MATLAB:badInput","dfX and dfY must be the same size.\n");
		}
		}
	}
...
getInverse<<<gridSize,blockSize>>>(XTex, YTex, hd_iX, hd_iY, dimX, dimY, dimZ, numVoxels, numIterations);
